...
function addMetaData(styles, data) {
  if (data instanceof Map) {
    ...
    if (data.has("class")) {
      addClassification(this, styles.meta, data.get("class"), data.get("description"));
    }
  }
  return styles;
}

function addClassification(ctx, meta, classification, description) {
  ...
  classifications.forEach(function(type) {
    ...
    if (!(isCompoundClassification || isCompoundModifier)) {
      // for each known classification...
      meta.classes.forEach(function(item) {
        // if it's the same...
        if (item.type === type) {
          // throw a warning
          console.warn("[restyle] a conflict was found for type `" + ctx.type[ctx.type.length - 1] + "`. It can't be both `" + modifier.join(" ") + "` and `" + item.modifier.join(" ") + "` as they both describe the " + type);
