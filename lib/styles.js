 ...
 */
Styles.prototype.processGrammars = function(grammars, weight) {
  ...
  grammars.forEach(function(grammar) {
    weight += Math.pow(WEIGHT_OFFSET, 2);
    // extract the type and description from the grammar
    var type = grammar.type || grammar.get("type");
    var description = grammar.description || grammar.get("description");
    ...
    var styles = this.extractStylesFromDefinition(component, description, weight);
  ...
  if (styles) {
    ...
    stylesMap = styles.reduce(function(map, item) {
      ...
      if (item.type === "variable") {
        ...
        value = createStylesMap(item.value, variables, map);
...
Styles.prototype.extractStylesFromAlias = function(alias, description) {
  ...
  return this.extractStylesFromDefinition(component, grammar.description, null, null);
};
 ...
 * @returns {Array<Object>} the styles
 */
Styles.prototype.extractStylesFromNested = function(nested, weight) {
  return this.extractStylesFromDefinition(nested, null, weight, null);
};
 ...
 * @returns {Array<Object>} the styles
 */
Styles.prototype.extractStylesFromDefinition = function(definition, description, weight, styles) {
  if (!definition) {
    return null;
  }
  var extras = {};
    ...
    if (RESTYLE_DIRECTIVE_PATTERN.test(key)) {
      ...
      if (!key || key === "restyle") {
        if (value) {
          ...
          mergeStyles(styles, this.processGrammars(grammars, weight));
        }
      }
      // if it's `remove`...
      else if (key === "remove") {
      ...
      // if it's `alias` or `inherit`...
      else if (key === "alias" || key === "inherit") {
      ...
      // if it's `states`...
        ...
        value.forEach(function(itemValue, itemKey) {
          ...
          styles.states.push({
            key: itemKey,
            value: this.extractStylesFromNested(itemValue, weight),
            weight: weight,
            type: "state"
          });
        }.bind(this));
      ...
      if (Styles.isSelector(key)) {
        value = this.extractStylesFromNested(value, weight);
        type = "selector";
      }
      // push the key-value
      styles.push({
