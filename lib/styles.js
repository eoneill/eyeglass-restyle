...
var DYNAMIC_MODIFIER_PATTERN = /\{([^\}]+)\}/g;
...
/**
  ...
  * @param    {Map} userVariables - user provided variable values
  * @returns  {Map} the resolved styles
  */
function Styles(grammars, allowedTypes, registeredComponents, aliases, contextStack, grammarEngines, moreSassUtils, userVariables) {
  ...
  if (userVariables) {
    userVariables.forEach(function(value, key) {
      variables[key] = value;
    });
  }
  styles = createStylesMap(styles, variables);
...
Styles.prototype.processGrammars = function(grammars) {
  ...
  grammars.forEach(function(grammar) {
    ...
    // if we have needed classifications, check those now...
    if (styles && styles.meta && styles.meta.neededClasses) {
      // get the current classifications as an array
      var classes = (styles.meta.classes || []).map(function(item) {
        return item.type;
      });
      // filter the ones that are missing
      var missingClasses = styles.meta.neededClasses.filter(function(classType) {
        // if it's not one of the current classes...
      ...
      // if we have any missing classifications...
      if (missingClasses.length) {
        // throw an error
        util.logger.error(
          "`" + type + "` needs a modifier that describes the follow attributes: " + missingClasses.join(", ")
        );
      }
    }
...
function mergeStyles(allStyles, styles) {
  ...
  if (styles) {
    ...
    Object.keys(preservedProps).forEach(function(prop) {
      var current = allStyles[prop];
      var previous = preservedProps[prop];

      // if it's an array, we should merge them
      if (Array.isArray(previous)) {
        current = current || [];
        current.push.apply(current, previous);
        allStyles[prop] = current;
      }
      // otherwise, if there's no current value, restore the previous value
      else if (!current && previous) {
        allStyles[prop] = previous;
      }
    });
  }
}
...
function createStylesMap(styles, variables, parent, proxy) {
  ...
  if (styles) {
    ...
    stylesMap = styles.reduce(function(map, item) {
      if (item.type === "variable") {
      ...
      else if (item.type === "variable-constraint") {
        variables.__constraints = variables.__constraints || {};
        variables.__constraints[item.key] = item.value;
      }
        ...
        if (Array.isArray(item.value) && Array.isArray(selectors[item.key])) {
          ...
          PRESERVED_PROPERTIES.forEach(function(prop) {
            if (Array.isArray(selectors[item.key][prop]) && Array.isArray(item.value[prop])) {
...
function resolveStyleValues(stylesMap) {
  ...
  function resolveVarPrefixValue(varName, map) {
    // validates values are within a given set of values (or the inverse)
    function validateValueIn(value, values, contains) {
      if (!values) {
        return;
      }
      // TODO - this should probably be cleaned up to do proper comparison
      var valueStr = value && value.toString();
      var isContained = values.some(function(val) {
        if (val && val.toString() === valueStr) {
          return true;
        }
      });
      if (isContained !== contains) {
        util.logger.error(
          "`" + varName + "` must " + (contains ? "" : "not ") + "be one of (" +
          values.join(", ") + "), but got `" + valueStr + "`"
        );
      }
    }

    map = map || stylesMap;
    var value = map.variables[varName];
    var constraints = map.variables.__constraints && map.variables.__constraints[varName];
    // if we have constraints and the value doesn't need to be resolved any further...
    if (constraints && !doesNeedPostResolution(value)) {
      validateValueIn(value, constraints.get("one-of"), true);
      validateValueIn(value, constraints.get("not-one-of"), false);
    }
    return value;
...
Styles.prototype.extractStylesFromDefinition = function(definition, description, styles, isPrivateAllowed) {
  ...
  someMap(definition, function(value, key) {
    ...
    if (Styles.isDirective(key)) {
      ...
      // if it's `var-constraints`...
      else if (key === "var-constraints") {
        value.forEach(function(itemValue, itemKey) {
          styles.push({
            key: itemKey,
            value: itemValue,
      ...
      // if it's `function()`...
        ...
        styles.push({
          key: RESTYLE_DIRECTIVE + "function-call\\" + key,
  ...
  if (extras.modifiers && description) {
    ...
    extras.modifiers.forEach(function(modifier, key) {
      ...
      var match = getMatch(modifierGrammar.description, description);
      // if it is a match...
      if (match.weight) {
        // get the styles from the modifier definition
        this.modifier.push(modifierGrammar.description);
        if (match.vars) {
          // merge in all the variables we computed from the dynamic modifiers
          Object.keys(match.vars).forEach(function(varName) {
            modifier.set("restyle-var(" + varName + ")", match.vars[varName]);
          });
        }
...
function addMetaData(styles, data) {
  if (data instanceof Map) {
    ...
    if (data.has("needs")) {
      addNeededClassifications(styles.meta, data.get("needs"));
    }
  }
  return styles;
}
...
/**
  * keeps track of the required (needed) classifications
  */
function addNeededClassifications(meta, needs) {
  meta.neededClasses = (meta.neededClasses || []).concat(needs);
}
...
/**
  * given a dynamic modifier, extracts the RegExp pattern and the variable names
  *
  * @param   {String} modifier - the dynamic modifier
  * @returns {Object} the pattern and varNames
  */
function getDynamicModifier(modifier) {
  var varNames = [];
  var pattern = new RegExp(
    util.escapeForRegExp(modifier.replace(DYNAMIC_MODIFIER_PATTERN, function(match, varName) {
...
/**
  * whether or not the modifier is dynamic
  *
  * @param   {String} modifier - the modifier to check
  * @returns {Boolean} whether or not it's dynamic
  */
function isDynamicModifier(modifier) {
  return DYNAMIC_MODIFIER_PATTERN.test(modifier);
}

/**
  * computes a "match" from a given haystack and needles
  *
  * @param   {Array<String>|String} needles - the needles to match for
  * @param   {Array} haystack - the haystack to match within
  * @returns {Object} the match, includes the weight and any dynamic variables
  */
function getMatch(needles, haystack) {
  var len = needles.length;
  var vars;

  // normalize the words in the haystack
  haystack = haystack.map(util.normalizeWord);

  var isMismatch = needles.some(function(needle) {
    // if it's a dynamic modifier...
    if (isDynamicModifier(needle)) {
      // get the pattern
      var dynamicModifier = getDynamicModifier(needle);
      // determine whether or not a matching pattern is in the haystack
      var isInHaystack = haystack.some(function(hay) {
        if (dynamicModifier.pattern.test(hay)) {
          var values = hay.match(dynamicModifier.pattern);
          values.slice(1).forEach(function(value, i) {
            vars = vars || {};
            var varName = dynamicModifier.varNames[i];
            vars[varName] = value;
          });
          return true;
        }
      });
      // if it's not in the haystack...
      if (!isInHaystack) {
        // return mismatch
        return true;
      }
    }
    // otherwise if it's not in the hastack
  ...
  // if it mismatched, return 0, otherwise, return the length (total matches)
  var weight = isMismatch ? 0 : (len * WEIGHT_OFFSET);
