  ...
  if (styles) {
    ...
    stylesMap = styles.reduce(function(map, item) {
      if (item.type === "variable") {
        ...
        else if (doesNeedPostNestedResolution(item.value, item.type)) {
          map.nestedKeysToResolve = map.nestedKeysToResolve || [];
          // stash it
          map.nestedKeysToResolve.push(item.key);
        }
    ...
    PRESERVED_PROPERTIES.forEach(function(prop) {
      ...
      if (styles[prop]) {
        if (prop === "states") {
          ...
          var fnMap = createStylesMap(styles.functions, variables, stylesMap, stylesMap);
          propObj = {
            calls: fnMap
          };
        }
...
function resolveStyleValues(stylesMap) {
  ...
  function resolveVarPrefixValue(val, map) {
    ...
    return map && (map.variables && map.variables[val] || (map.parent ? resolveVarPrefixValue(val, map.parent) : undefined));
  }
  ...
  if (stylesMap) {
    ...
    if (stylesMap.nestedKeysToResolve && stylesMap.nestedKeysToResolve.length) {
      stylesMap.nestedKeysToResolve.forEach(function(k) {
        ...
        else if (Styles.isSelector(k) || val.isMultiValue) {
 ...
 */
function doesNeedPostNestedResolution(value, type) {
  return !!(value && value instanceof Map && (Styles.isDirective(type) || Styles.isSelector(type)));
}
