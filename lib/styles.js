...
var RESTYLE_VAR_DIRECTIVE_PATTERN = /^var(?:(?:\s+|\\)(\S+)|\(\s*(\S+)\s*\))/;
var RESTYLE_FUNCTION_DIRECTIVE_PATTERN = /^function(?:(?:\s+|\\)(\S+)|\(\s*(\S+)\s*\))/;
var RESTYLE_MODIFIER_DIRECTIVE_PATTERN = /^modifier(?:(?:\s+|\\)(\S+)|\(\s*(\S+)\s*\))/;
var RESTYLE_STATE_DIRECTIVE_PATTERN = /^state(?:(?:\s+|\\)(\S+)|\(\s*(\S+)\s*\))/;
...
var PRESERVED_PROPERTIES = ["states", "functions", "meta"];
 ...
 */
function Styles(grammars, allowedTypes, registeredComponents, aliases, grammarEngines) {
...
Styles.prototype.processGrammars = function(grammars) {
  ...
  grammars.forEach(function(grammar) {
    ...
    if (this.allowedTypes.indexOf(type) === -1) {
      // TODO - do we need this or does Grammar already handle this error for us?
      ...
      if (styles[prop]) {
        if (prop === "states") {
          styles.states.forEach(function(state) {
            if (state.value) {
              var functions = state.value.functions;
              propObj[state.key] = (propObj[state.key] || []).concat(state.value);

              // preserve nested functions
              if (functions) {
                propObj[state.key].functions = (propObj[state.key].functions || []).concat(functions);
              }
            }
            else {
              propObj[state.key] = state.value;
            }
    ...
    value.some(function(item) {
      ...
      if (typeof item === "string" && pattern.test(item)) {
        ...
        if (Array.isArray(item)) {
          ...
          canUpdate = true;
          // run the resolver function to get the resolved value
          item = item.map(function(deepItem) {
            var rndKey = Math.random();
            resolveStyleValue(pattern, resolverFn, deepItem, rndKey, stylesMap);
            resolvedItem = stylesMap.get(rndKey);
            stylesMap.set(rndKey);
            return resolvedItem || deepItem;
          });
          item.sassSeparator = nestedArraySeparator;
        }
...
Styles.prototype.extractStylesFromAlias = function(alias, description) {
  ...
  // get the grammar
  var grammar = new Grammar(description, null, this.allowedTypes, this.aliases, this.grammarEngines);
...
Styles.prototype.extractStylesFromDefinition = function(definition, description, styles) {
  ...
  someMap(definition, function(value, key) {
    ...
    if (Styles.isDirective(key)) {
      ...
      if (!key || key === "restyle") {
        if (value) {
          ...
          grammars = grammars.map(function(grammar) {
            return new Grammar(grammar, null, this.allowedTypes, this.aliases, this.grammarEngines);
          }.bind(this));
      ...
      // if it's `inherit` or `extends`...
        ...
        value = (typeof value === "string") ? currentItem(this.type) + " " + value : value.shift(currentItem(this.type));
  ...
  if (extras.modifiers && description) {
    ...
    extras.modifiers.forEach(function(modifier, key) {
      var modifierGrammar = new Grammar(key, Grammar.NO_TYPE, this.allowedTypes, this.aliases, this.grammarEngines);
...
function doesNeedPostNestedResolution(value, key) {
  return !!(value && value instanceof Map && (Styles.isDirective(key) || Styles.isSelector(key)));
}
...
function addClassification(ctx, meta, classification, description, allowConflicts) {
  var modifier = ctx.modifier[ctx.modifier.length - 1];
    ...
    if (!(isCompoundClassification || isCompoundModifier || allowConflicts)) {
      ...
      meta.classes.forEach(function(item) {
        ...
        if (item.type === type && !item.allowConflicts) {
          // throw a warning
          console.warn("[restyle] a conflict was found for `" + ctx.type[ctx.type.length - 1] + "`. It can't be both `" + modifier.join(" ") + "` and `" + item.modifier.join(" ") + "` as they both describe the " + type);
          if (description && item.description) {
            console.warn("\t" + modifier.join(" ") + ": " + (Array.isArray(description) ? description.join(" ") : description));
            console.warn("\t" + item.modifier.join(" ") + ": " + (Array.isArray(item.description) ? item.description.join(" ") : item.description));
          }
        }
      });
    }
...
function removeStyles(styles, props) {
  ...
  if (states) {
    ...
    styles.states = states.filter(function(state) {
      if (isPropsArray) {
        return !props.some(function(prop) {
          if (STATES_DIRECTIVE_PATTERN.test(prop)) {
            prop = prop.replace(STATES_DIRECTIVE_PATTERN, "$1");
            if (state.key === prop) {
              return true;
            }
          }
      ...
      return !(STATES_DIRECTIVE_PATTERN.test(props) && props.replace(STATES_DIRECTIVE_PATTERN, "$1") === state.key);
    });
  }
