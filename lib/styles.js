...
function constructDirectivePattern(directive) {
  return new RegExp("^(?:restyle-)*(?:(?:@" + directive + "\\.|" + directive + "\\\\)(\\S+)|" + directive + "\\(\\s*(\\S+)\\s*\\))");
}

var THIS_DIRECTIVE_PATTERN = constructDirectivePattern("this");
var VAR_DIRECTIVE_PATTERN = constructDirectivePattern("var");
var ROOT_DIRECTIVE_PATTERN = constructDirectivePattern("root");
var PARENT_DIRECTIVE_PATTERN = constructDirectivePattern("parent");
var STATES_DIRECTIVE_PATTERN = constructDirectivePattern("states");
  ...
  if (stylesMap) {
    ...
    if (stylesMap.nestedKeysToResolve && stylesMap.nestedKeysToResolve.length) {
      stylesMap.nestedKeysToResolve.forEach(function(k) {
        ...
        if (val) {
          if (RESTYLE_DIRECTIVE_PREFIX_PATTERN.test(k)) {
...
Styles.prototype.extractStylesFromDefinition = function(definition, description, styles) {
  ...
  someMap(definition, function(value, key) {
    ...
    if (RESTYLE_DIRECTIVE_PREFIX_PATTERN.test(key)) {
      // strip the prefix
      key = key.replace(RESTYLE_DIRECTIVE_PREFIX_PATTERN, "");
...
function doesNeedPostNestedResolution(value, key) {
  return !!(value && (RESTYLE_DIRECTIVE_PREFIX_PATTERN.test(key) || Styles.isSelector(key)));
}
